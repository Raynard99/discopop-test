...
string CUGeneration::determineVariableName(
  ...
  if (operand->hasName()) {
    ...
    if (isa<GetElementPtrInst>(*operand)) {
      ...
      if (structType && gep->getNumOperands() > 2) {
        ...
        if (constValue && isa<ConstantInt>(*constValue)) {
          ...
          if (!(cast<StructType>(structType))->isLiteral()) {
            ...
            if (it != Structs.end()) {
              std::string ret =
                  findStructMemberName(it->second, memberIdx, builder);
              if (ret.size() > 0)
                return ret;
              else
                return getOrInsertVarName("", builder);
      ...
      // we've found an array
      if (PTy->getElementType()->getTypeID() == Type::ArrayTyID &&
          isa<GetElementPtrInst>(*ptrOperand)) {
...
string CUGeneration::findStructMemberName(MDNode *structNode, unsigned idx,
                                          IRBuilder<> &builder) {
  assert(structNode);
  ...
  if (idx < memberListNodes->getNumOperands()) {
    ...
    if (member->getOperand(3)) {
      // getOrInsertVarName(string(member->getOperand(3)->getName().data()),
      // builder); return string(member->getOperand(3)->getName().data());
      getOrInsertVarName(dyn_cast<MDString>(member->getOperand(3))->getString(),
                         builder);
...
void CUGeneration::populateGlobalVariablesSet(Region *TopRegion,
...
void CUGeneration::createCUs(Region *TopRegion, set<string> &globalVariablesSet,
                             vector<CU *> &CUVector,
...
void CUGeneration::fillCUVariables(Region *TopRegion,
                                   set<string> &globalVariablesSet,
                                   vector<CU *> &CUVector,
                                   map<string, vector<CU *>> &BBIDToCUIDsMap) {
  int lid;
...
static RegisterPass<CUGeneration>
    X("CUGeneration", "CUGeneration: determine computation units.", false,
      false);
...
static RegisterStandardPasses
    CUGenerationLoader_Ox(PassManagerBuilder::EP_OptimizerLast, loadPass);
static RegisterStandardPasses
    CUGenerationLoader_O0(PassManagerBuilder::EP_EnabledOnOptLevel0, loadPass);
...
StringRef CUGeneration::getPassName() const { return "CUGeneration"; }
